...
export class FirecrawlFetchProvider {
    ...
    async fetchWithApiKey(request, apiKey, signal) {
        let response;
        try {
    ...
    toWebError(error, signal) {
        if (error instanceof WebError)
            return error;
        if (error instanceof FirecrawlCredentialMissingError) {
            return new WebError('Firecrawl fetch has no configured API key; store one through the credentials service or launch environment', 'WEB_PROVIDER_CREDENTIAL_MISSING');
        }
        ...
        if (signal?.aborted === true || isAbortError(error))
            return fetchAborted(signal, error);
